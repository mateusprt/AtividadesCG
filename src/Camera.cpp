...
float yaw   = -90.0f; // olhar para frente no eixo Z
float pitch = 0.0f;   // sem inclinação
float sensitivity = 0.1f; // ou 0.1f se usar mouse
...
int main()
{
  ...
  //Matriz de projeção
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Atualizar a matriz de projection
		glm::mat4 projection = glm::perspective(glm::radians(fov), (float)WIDTH/HEIGHT, 0.1f, 100.0f);
		glUniformMatrix4fv(glGetUniformLocation(shader.ID, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    // Limita o pitch entre -89 e 89 graus
    if (pitch > 89.0f)
        pitch = 89.0f;
    if (pitch < -89.0f)
        pitch = -89.0f;

    // Calcula o novo vetor direção
    glm::vec3 front;
    ...
    // Atualiza o vetor up (opcional, se quiser maior precisão com roll)
